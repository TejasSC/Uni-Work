public abstract class Cache{
  ...
  public void write(int addr, Object indata) {
    total_accesses++;
    write_accesses++;
    Object entry = cacheSearch(addr);//find if item addressed by addr is in cache
    if (entry != null) cacheWriteData(indata);//update cache data
    else {
      //System.out.println("Entry should be null here");
      write_misses++;//do nothing, another write miss
    }
  }

  public Object read(int addr) {
    total_accesses++;//one more access and read access in particular
    read_accesses++;
    Object entry = cacheSearch(addr);//find if item addressed by addr is in cache
    if (entry != null) return entry;//cache hit; it's in the cache
    else {
      read_misses++;//must search RAM
      //System.out.printf("Read miss %d\n", read_misses);
      main_mem_accesses++;
