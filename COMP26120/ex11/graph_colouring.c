...
minisat_Var getVar(int index, int colour) {
  ...
  return 0.5*((index+colour)*(index+colour+1)) + colour;
}//getVar
...
void addColouringConstraint(int n)
{
  ...
  minisat_Var* someVars = (minisat_Var*) malloc(colours*sizeof(minisat_Var));
  int i;
  for (i = 0; i < colours; i++){
    someVars[i] = getVar(n, i);
  }//for
  atLeastOneOfThese(someVars, colours);
  // Should not be two colours
  // Foreach pair of colours c1 and c2
  // ~(n is c1 and n is c2) = ~(n is c1) or ~(n is c2)
  int c1, c2;
  for (c1 = 0; c1 < colours; c1++) {
    for (c2 = 0; c2 < colours; c2++) {
      if (c1 != c2) {
        notBothOfThese(getVar(n, c1), getVar(n, c2));
      }//if
    }//for
  }//for
}//addColouringConstraint
...
{
  int c;
  for (c = 0; c < graph->MaxSize; c++) {
    List* someList = graph->table[c].outlist;
    while (someList!=NULL) {
      if (someList->index != c) {
        if (colouring[someList->index]==colouring[c]) {
          return 0; //return if any adjacent nodes have the same colours
        }//if
      }//if
      someList = someList->next;
    }//while
  }//for
  return 1; //every pair of adjacent nodes has different colours
}//checkColouring
