...
void branch_and_bound(int *final_sol)
{
  ...
  parent.fixed = 0;//solution is ****...
  for (int i = 1; i <= Nitems; i++) {
    parent.solution_vec[i] = 0;
  }//for
  // parent.fixed =0;
  frac_bound(&parent, parent.fixed); // compute its value and its bound
  current_best = parent.val; // put current_best = to its value
  insert(parent); // store it in the priority queue

  // int fixed = 0;

  // LOOP until queue is empty or upper bound is not greater than current_best:
  while (QueueSize!=0 && parent.bound > current_best /*&& fixed<=Nitems*/) {
    // *parent=removeMax();//remove the first item in the queue
    parent = removeMax();
    //construct two children, 1 with a 1 added, 1 with a O added
    struc_sol added_0;
    copy_array(parent.solution_vec, added_0.solution_vec);//get array from parent
    added_0.fixed = parent.fixed + 1;
    added_0.solution_vec[added_0.fixed] = 0;//add a 0

    struc_sol added_1;
    copy_array(parent.solution_vec, added_1.solution_vec);//get array from parent
    added_1.fixed = parent.fixed + 1;
    added_1.solution_vec[added_1.fixed] = 1;//add a 1

    //calculate upper bounds and values for the children
    //   FOREACH CHILD:
    ...
    // RETURN
    frac_bound(&added_0, added_0.fixed);
    if(added_0.val!=-1 /*&& added_0.val > current_best*/) {
      insert(added_0);
      if(added_0.val > current_best)
      {
        current_best = added_0.val;
        copy_array(added_0.solution_vec, final_sol);
      }
      // insert(added_0);
    }//if
    frac_bound(&added_1, added_1.fixed);
    if (added_1.val!=-1 /*&& added_1.val > current_best*/) {
      insert(added_1);
      if(added_1.val > current_best)
      {
        current_best = added_1.val;
        copy_array(added_1.solution_vec, final_sol);
      }
      // insert(added_1);
    }//if
  }//while
